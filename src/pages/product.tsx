...
import {
  ...
  getProductPriceWithoutVariantsDiscount,
...
export const generateOrderId = customAlphabet(
...
function Product({
  ...
  const [shipping, setShipping] = useState<ShippingInfo>({
    name: "",
    phone: "",
    doorShipping: store.metadata?.shipping?.mode == "deskOnly" ? false : true,
    address: {
      street: "",
      city: "1",
      location: {
  ...
  function getPriceWithoutVariantsDiscount(): number {
    return getProductPriceWithoutVariantsDiscount(product!, item.variants);
  }

  function getPriceAfterDiscount(): number {
    return getProductPriceAfterDiscount(product!, item.variants);
  }

  function getQuantity(): number {
    return getProductQuantity(product!, item.variants);
  }
  ...
  function updateShippingWilaya(stateCode: string) {
    ...
    if (!!shipping.name && !!shipping.phone && !localStorage.addedToCard) {
      ...
      track("AddToCart", {
        contents: [{ id: product?.id, quantity: getQuantity() }],
  ...
  function getDiscount() {
    return (
      100 -
      getProductDiscountPercentage(product!, item.variants) * 100
    ).toFixed(1);
  }
  ...
  async function sendOrder(status: "draft" | "pending" = "pending") {
    // console.log("sending...");
    var validationError = validatePhoneNumber(
    ...
    var data: any = {
      ...
      status: status,
      storeId: store.id,
      items: cart.hasProduct(product.id)
        ? cart.items
        : [
            {
    ...
    return (
      ...
      >
        ...
        <div className="flex items-center justify-center">
          ...
          <span
            dir="ltr"
            className="mx-2 text-primary rounded-full px-2"
            style={{ backgroundColor: "var(--on-p)" }}
          >
            x{item.quantity}
          </span>
        </div>
                    ...
                    !import.meta.env.SSR ? (
                      ...
                      >
                        {(mountPlayer && (
                          ...
                          <img
                            src={`https://img.youtube.com/vi/${getYoutubeVideoIdFromUrl(
                              media
                            )}/maxresdefault.jpg`}
                ...
                >
                  <button
                    ...
                    className={
                      ...
                      (selectedMediaIndex === index
                        ? "border-primary border-[2px] w-16"
                        : " w-14 border-[2px] dark:border-white border-white ") +
            ...
            <div className="product-color">
              ...
              <div id="order-form" className="gb rounded-xl">
                ...
                <div className="p-4 pt-1">
                          ...
                          {cart.items.length > 0 ? (
                            cart.items.map((_item) => (
                              ...
                              >
                                ...
                                <td>
                                  <img
                                    src={_item.product.media[0]}
                                    className="w-8 h-8 rounded-lg border-2 border-gray-200"
                                  />
                                </td>
                                <td className="text-gray-600">
                                  {_item.product.name &&
                                  _item.product.name.length > 10
                                    ? _item.product.name.substring(0, 10) +
                                      "..."
                                    : _item.product.name}
                                </td>
                                <td className="text-gray-600">
                            ...
                            <tr className="text-gray-600 text-center">
                              <td colSpan={4} className="text-xs">
                                لا يوجد منتجات في السلة | إضغط على شراء وسترسل
                                هذ المنتج فقط
                              </td>
                            </tr>
                          )}
                  ...
                  {/* shipping */}
                  <div className="flex items-center justify-center">
                    <div className="text-gray-600">التوصيل</div>
                    <div className="flex-grow"></div>
                          ...
                          <span>
                            {
                              ...
                              (() => {
                                ...
                                if (false && cart.hasProduct(product.id)) {
                                  rate = cart.getShippingRate(shipping, store);
                                } else {
